...
export default function RatingsPage() {
  ...
  useEffect(() => {
    fetch("/api/ratings/")
      ...
      .then((d) => {
        ...
        if (d.doctors[0]) setSelected(d.doctors[0].id);
      });
  }, []);
  ...
  const currentDoctor = doctors.find((d) => d.id === selected);
        ...
        {detail && currentDoctor && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <Card>
                <p className="text-sm font-medium">Patient Feedback ({detail.patientCount})</p>
                <div className="mt-4 space-y-3">
                  {Object.entries(PATIENT_DIM_LABELS).map(([key, label]) => (
                    <Bar key={key} label={label} value={detail.patientAvg[key] ?? 0} color="var(--forest)" />
                  ))}
                </div>
              </Card>
              <Card>
                <p className="text-sm font-medium">Dept Head Review ({detail.deptCount})</p>
                <div className="mt-4 space-y-3">
                  {Object.entries(DEPT_DIM_LABELS).map(([key, label]) => (
                    <Bar key={key} label={label} value={detail.deptAvg[key] ?? 0} color="var(--clay)" />
                  ))}
                </div>
              </Card>
            </div>
